...
Future<RecommendResponse> getTopCart([int id = 1]) async {
...
Future<List<PlaylistModel>> getTopPlaylist([
  ...
  var response = (await createRequest<Map<String, dynamic>>(
    RequestOptions(
  ...
  var responseList = response.body?['data']['special_list'];
  List<PlaylistModel> playlistModelList = [];
  if (responseList is List) {
    playlistModelList = responseList
...
class PlaylistModel {
  ...
  factory PlaylistModel.fromJson(Map<String, dynamic> json) {
    ...
    List<Tag>? tags = tagsValue is List
        ...
        : tagsValue is String
        ? tagsValue.split(',').map((name) => Tag(tagName: name)).toList()
        : null;
    return PlaylistModel(
      sync: json['sync'] as int?,
      specialid: json['specialid'] as int?,
      flexibleCover: json['flexible_cover'] is String
          ? getImageUri(json['flexible_cover'])
          : null,
